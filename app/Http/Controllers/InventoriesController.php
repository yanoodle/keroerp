...
class InventoriesController extends Controller
{
    ...
    public function index()
    {
        ...
        $lowStockProducts = Products::where('base_qty', '<=', 10)->get();
    ...
    { {
            $request->validate([
                ...
                'base_qty' => 'required|numeric|min:1|max:200',
                'in_demand_qty' => 'required|numeric|min:1',
    ...
    { {
            $request->validate([
                'name' => 'required',
                'description' => 'required',
                'base_qty' => 'required|numeric|min:0|max:200',
                'in_demand_qty' => 'required|numeric|min:0|max:200',
                'vendor_name' => 'required',
                'vendor_contact' => 'required',
                'sell_price' => 'required',
                'buy_price' => 'required'
    ...
    {
        // Check for existing sales entries
        $hasSales = Sales::where('product', $inventory->name)->exists();

        // Check for existing purchasing entries
        $hasPurchases = Purchasing::where('product', $inventory->name)->exists();

        if ($hasSales || $hasPurchases) {
            return back()->withErrors([
                'msg' => 'Cannot delete this item because it has associated sales or purchasing records.'
            ]);
        }
